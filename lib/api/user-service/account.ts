...
export const accountApi = {
    ...
    getAccount: async (): Promise<UserAccount | null> => {
        try {
            const response = await authenticatedFetch(getMicroserviceUrl("user-service", "/api/v1/account"))
    ...
    uploadProfileImage: async (file: File): Promise<{ success: boolean, url?: string, message?: string }> => {
        try {
            ...
            const updateResult = await authenticatedFetch(getMicroserviceUrl("user-service", "/api/v1/account"), {
                method: "PATCH",
                body: JSON.stringify({
    ...
    deleteProfileImage: async (): Promise<{ success: boolean, message?: string }> => {
        try {
            const response = await authenticatedFetch(getMicroserviceUrl("user-service", "/api/v1/account/profile-image"), {
                method: "DELETE"
            })
